...
@interface NotEqualMaginController ()

@end

@implementation NotEqualMaginController
...
- (void)viewDidLoad {
  ...
  [@[ yellowView1, yellowView2, yellowView3 ]
      mas_distributeViewsAlongAxis:MASAxisTypeHorizontal
                  withFixedSpacing:padding
                       leadSpacing:padding
                       tailSpacing:padding];
  ...
  [blueViews mas_distributeViewsAlongAxis:MASAxisTypeHorizontal
                      withFixedItemLength:30
                              leadSpacing:padding2
                              tailSpacing:padding2];
  ...
  [redView distributeViewsAlongAxis:MASAxisTypeHorizontal
              withSpaceWeightArrays:@[ @2, @0, @0.8, @0 ]
                   andSubViewsArray:greenViews];
  //  [redView distributeSpacingHorizontallyWith:greenViews];

  //在红色View里面放三个大小不一样的绿色正方形, 间隙等大,
  // masonry并没提供相关方法
  NSMutableArray* greyViews = [NSMutableArray array];
  for (NSInteger i = 0; i < 3; i++) {
    UIView* greyView = [[UIView alloc] init];
    greyView.backgroundColor = [UIColor darkGrayColor];
    [redView addSubview:greyView];
    [greyViews addObject:greyView];
    [greyView mas_makeConstraints:^(MASConstraintMaker* make) {
      make.right.equalTo(redView).offset(-10);
      make.height.mas_equalTo(i * 20 + 20);
      make.width.mas_equalTo(greyView.mas_height);
    }];
  }

  [redView distributeViewsAlongAxis:MASAxisTypeVertical
              withSpaceWeightArrays:@[ @0.9, @06, @0, @8 ]
                   andSubViewsArray:greyViews];
}
